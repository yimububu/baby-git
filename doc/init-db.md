...
3. 如果环境变量已设置且路径存在并且是目录，当前实现会直接 `return 1` 提前结束。
4. 由于提前结束，后续对象库初始化不会执行：不会创建 `objects`，也不会创建 `00..ff` 子目录。
5. 如果环境变量路径不可用（不是有效目录），会打印错误提示并回退到默认路径 `.dircache/objects`。
6. 在默认路径分支中，创建对象库目录，并在其下创建 `00` 到 `ff` 共 256 个子目录。

这 256 个子目录对应 SHA-1 十六进制前两位分桶。后续对象文件会按哈希前缀落入对应目录，例如哈希以 `ab` 开头就写入
`objects/ab/`。这样避免把海量对象放进同一个目录，显著减轻目录项查找和管理压力。

注意：上面的第 3-4 步是这份早期实现的行为缺陷。也就是说，`init-db` 并不能在 `SHA1_FILE_DIRECTORY` 指向有效目录时完成该目录下的初始化。
